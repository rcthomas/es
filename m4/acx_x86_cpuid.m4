dnl @synopsis ACX_X86_CPUID(OP)
dnl
dnl @summary run x86 cpuid instruction OP using inline assembler
dnl
...
dnl variable acx_cv_x86_cpuid_OP.
dnl
...
dnl ax_cv_x86_cpuid_OP is set to the string "unknown".
dnl
dnl This macro mainly exists to be used in ACX_X86_ARCHFLAG.
dnl
dnl @category Misc
...
dnl 2008-12-10
dnl All major compilers (gcc, pathscale, intel, pgi, XL, etc)
dnl support inline assembly in this format.  So remove all references
dnl to "GCC"

AC_DEFUN([ACX_X86_CPUID],
[AC_REQUIRE([AC_PROG_CXX])
AC_LANG_PUSH([C++])
AC_CACHE_CHECK(for x86 cpuid $1 output, acx_cv_x86_cpuid_$1,
 [AC_RUN_IFELSE([AC_LANG_PROGRAM([#include <cstdio>], [
     ...
     [acx_cv_x86_cpuid_$1=`cat conftest_cpuid`; rm -f conftest_cpuid],
     [acx_cv_x86_cpuid_$1=unknown; rm -f conftest_cpuid],
     [acx_cv_x86_cpuid_$1=unknown])])
AC_LANG_POP([C++])
])
