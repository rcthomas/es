/* $Id: example2a.cpp,v 1.1 2006/06/30 01:07:26 tgkolda Exp $ */
/* $Source: /space/CVS-Acro/acro/packages/appspack/appspack/examples/example2a.cpp,v $ */
...
/*! \file example2a.cpp
  ...
  now handled using APPSPACK's built-in extreme barrier approach.

  A simple example of a function evaluation that takes its input from
  a text file, and it writes a single objective value and its
  constraint value to an output file, one entry per line.
  ...
  The input file example2a.apps, to be used with example2a.cpp, includes
  ...
  its function value and its constraint value to the output file. This
  special case can be easily handled via the
  APPSPACK::Combiner::Generic function that combines multiple function
