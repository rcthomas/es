...
ES::Synow::Grid::Grid( double const min_wl_, double const max_wl_, double const bin_width_, size_t const v_size_ ) :
...
void ES::Synow::Grid::reset( ES::Synow::Setup& setup )
{
    ...
    for( size_t i = 1; i < v_size - 1; ++ i ) v [ i ] = setup.v_phot + i * v_step;
    v[ 0 ] = setup.v_phot;
...
{
    wl_used = 0;
    for( size_t i = 0; i < wl_size * v_size; ++ i )
    {
